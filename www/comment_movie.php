...
<?php 
    ...
    if ($code == "") {
      ...
      $query_movie = $db->query("SELECT id, title, year FROM Movie WHERE id = $code");
      if (!$query_movie) {
          $errmsg = $db->error;
          echo "error";
...
<div class="container">
  <h2 class="text-center">Add Comment For This Movie</h2>
  <br>
  <hr>
  <br>
  <form class="form-horizontal" method = "get" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
    ...
    <div class="form-group">        
      <div class="col-sm-offset-2 col-sm-10">
        ...
        <a type="button" class="btn btn-warning" href="./show_movie.php?code=<?php
        if ($_GET["title"] != "") {
          echo $_GET["title"];          
        } else {
          echo $_GET["code"];          
        ...
        ?>">BACK</a>
        <a type="button" class="btn btn-secondery" href="./comment_movie.php?code=<?php
        if ($_GET["title"] != "") {
          echo $_GET["title"];          
        } else {
          echo $_GET["code"];          
...
<div class="container">
  ...
  if ($title == "" && $rating == "" && $comment == "" && $myname == "") {
    ...
    if (!$db->query($query)) {
        ...
        echo "<br>";
        echo "Plese Go BACK to View Your Post!!!";
      } 
